...
void *check_subgrid(void *arg)
{
    ...
    for (int i = row; i < row + 3; i++){
        int before[9] = {0,}; // 현재 숫자가 이전에 나왔는지를 체크하는 배열
        int is_valid = 1; // for loop를 빠져나온 뒤, 서브그리드가 올바른지 판단할 때 사용하는 변수
        ...
        if (is_valid) // k번 서브그리드가 올바름
            valid[2][k] = 1; 
        else // k번 서브그리드가 올바르지 않음
            valid[2][k] = 0; 
    }
    // k번 서브그리드 체크 완료
    pthread_exit(NULL);
...
void check_sudoku(void)
{
     ...
     */
    // 여기를 완성하세요
    pthread_create(&rowtid, 0, check_rows, NULL);

    /*
     * 스레드를 생성하여 각 열을 검사하는 check_columns() 함수를 실행한다.
     */
    // 여기를 완성하세요
    pthread_create(&coltid, 0, check_columns, NULL);
    ...
    for (i = 0; i <= 6; i = i + 3){
        for (j = 0; j <= 6; j = j + 3){
            ...
            arg -> row = i;
            arg -> col = j;
            pthread_create(&subgridtid[k++], 0, check_subgrid, arg);
        }
    }
